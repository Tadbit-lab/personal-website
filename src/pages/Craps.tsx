...
function Craps() {
  ...
  useEffect(() => {
    // 1. Preload audio
    ;['dice-roll', 'dice-hit', 'win', 'lose', 'welcome'].forEach((name) => {
    ...
    async function setupDice() {
      try {
        // Brief delay allows React DOM node to settle dimensions
        ...
        if (isMounted) setDiceReady(true) // Enable button anyway so game is playable
      }
    }
  ...
  const roll = async () => {
    if (rolling || balance < amount) return
    setRolling(true)
    play('dice-roll')
    ...
    if (bet === 'field') {
      ...
      if ([7, 11].includes(total)) won = bet === 'pass'
      else if ([2, 3].includes(total)) won = bet === 'dontpass'
      else if (total === 12) won = bet === 'dontpass' ? null : false
      else {
        setPoint(total)
        setPhase('point')

...
import { Chart as ChartJS, TimeScale, LinearScale, Tooltip, Legend, type ChartData, type ChartDataset, type ChartOptions } from 'chart.js'
...
function StockChart({ values, labels, symbol, timeframe, loading: propLoading = false, error: propError = null }: StockChartProps) {
  ...
  const data = useMemo<ChartData<'candlestick', CandlestickPoint[], unknown>>(() => ({
    datasets: [{
    ...
    } as unknown as ChartDataset<'candlestick', CandlestickPoint[]>],
  ...
  const options = useMemo<ChartOptions<'candlestick'>>(() => ({
    ...
    plugins: {
      ...
      tooltip: {
        ...
        bodyColor: '#d1d5db',
        callbacks: {
          label: (context: { raw: CandlestickPoint }) => {
  ...
  return (
    <div className="chart-wrap" style={{ height: '100%', background: '#0b1220', padding: '12px', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px' }}>

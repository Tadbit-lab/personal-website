...
function drawCandlesticks(canvas, candles) {
    ...
    const toY = (price) => PAD.top + chartH - ((price - paddedMin) / paddedRange) * chartH;
...
function StockChart({ values, labels, symbol, timeframe, loading: propLoading = false, error: propError = null }) {
    ...
    useEffect(() => {
        ...
        if (!canvas || !resolvedCandles.length)
            return;
    ...
    return (_jsx("div", { ref: containerRef, className: "chart-wrap", style: { height: '100%', background: '#0b1220', padding: '12px', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px' }, children: _jsx("canvas", { ref: canvasRef, style: { width: '100%', height: '100%', display: 'block' } }) }));
}
